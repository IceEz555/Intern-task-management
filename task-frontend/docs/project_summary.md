...
- **Frontend (`ProfilePage.jsx`)**:
    - **View**: แสดงข้อมูลส่วนตัวและรูป Avatar
    - **Edit**: ฟอร์มแก้ไขชื่อ, แผนก, เบอร์โทร เชื่อมต่อ `PUT /api/users/profile`
    - **UX**: ใช้ `AuthContext` อัปเดต UI ส่วนอื่นๆ (เช่น Topbar) ทันทีที่บันทึกเสร็จ
- **Backend**:
    - เพิ่ม `department`, `phone` ในตาราง `users`
    - API `PUT /api/users/profile` สำหรับแก้ไขข้อมูลตัวเอง

### 📊 Admin Dashboard Refinement
- **Stats Integration**: เชื่อมต่อ API `GET /api/admin/stats` แสดงตัวเลขจริง
- **Dynamic Chart**: กราฟ Donut แสดงสัดส่วน User ตาม Role โดยใช้ CSS Conic Gradient (ไม่ต้องลง Library เพิ่ม)
- **UI/UX**: ปรับโทนสีพาสเทล (Blue, Purple, Light Blue) ตามที่รีเควส
